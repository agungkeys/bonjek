    ...
    <section class="pt-3" style="margin-bottom: 5em;">
      <ul class="nav nav-tabs" id="myTab" role="tablist">
        <li class="nav-item waves-effect waves-light">
            @if(!empty($url))
                <a class="nav-link" id="profile-tab" href="/profile/{{Auth::user()->id}}/edit?{{$url}}" aria-selected="false">Profil</a>
            @else
      ...
      <div class="tab-content" id="myTabContent">
        <div class="tab-pane fade active show" id="profile-detail" role="tabpanel" aria-labelledby="profile-detail-tab">
          <form method="post" action="{{ route('profile-detail.store') }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
              <label>Bio</label>
              <input type="text" name="bio" id="bio" class="form-control" placeholder="Biografi"></input>
            </div>
